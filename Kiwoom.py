...
class Kiwoom(QAxWidget, Write_db):
    ...
    def update_hold_list(self, code, remain_qty, buy_price, order_num, fee):
        df = read_sql("SELECT * FROM 'order_list' WHERE symbol='{}'".format(code), self.con, index_col='tid')
    ...
    # 계좌번호 (위탁종합) 리턴
    def getAccount(self):
        n_acc = int(self.getLoginInfo("ACCOUNT_CNT"))
        accountList = self.getLoginInfo("ACCNO").split(';')
        return accountList[:n_acc][0]
    ...
    def getBalance(self):
        ...
        return int(self.opw00018Data["accountEvaluation"][-1].replace(',', '')) - int(self.opw00001Data["accountEvaluation"][1].replace(',', ''))

    # 일봉 데이터 다운
    def req_day_data(self, code, repeat):
        self.repeat = repeat
        self.dfs = []
